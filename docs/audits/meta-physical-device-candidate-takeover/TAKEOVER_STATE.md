...
No pushes were performed. No mobile files were modified. All work described above and below was committed locally on `feature/meta-physical-device-candidate-v1` (commits `9311442`, `4552644`).

## Addendum: HUD wired to the real backend (commit `22b44aa`)

Per user direction after the initial audit, the Meta HUD's live runtime was wired to the real backend, and the mobile pairing-UI decision was deferred (see the AskUserQuestion answers this session: retire the mobile branch's invented backend and re-point to the live contract; wire the HUD now; commit locally now — all three accepted).

**What was built:** `companion-real.html` + `src/companion/realCompanionEntry.js` — a new PRIVATE HARDWARE CANDIDATE QA page, structurally parallel to the existing mock `companion.html`/`mockCompanion.js`, but driven by `createPhoneCompanion` (the real backend integration state machine) behind a real Supabase sign-in rather than canned fixtures. It reuses the exact same `createParentWindowTransport` the HUD already uses, from the parent-window side — no HUD/`companionRuntime.js` changes were needed, confirming Phase A's own "adapter seam" design held up. The page is excluded from all three shipping builds by construction (never referenced in any `vite.*.config.js` `rollupOptions.input`), which a new `static-tests.js` section (`J.real-companion:*`) now asserts.

**Two real, previously-undetected defects in `phoneCompanion.js` were found and fixed** by actually loading this page in a browser (a fresh, no-cache tab) and reading the console — something `test:wearable`'s unit tests never exercise, since they only import `resultFormatter.js` and run static checks, never `phoneCompanion.js` itself:

1. **Hard import error.** `phoneCompanion.js` imported `createPairingChallenge`, `approvePairingChallenge`, and `revokePairing` from `wearableBackend.js` — none of which exist. The real exports are `approvePairingByCode`, `denyPairingByCode`, `revokeWearableSession`, etc. This alone would have crashed the module on load in any real usage.
2. **Wrong pairing flow, not just wrong names.** `phoneCompanion.js` assumed a single phone-side call both approves a pairing and returns a session. The deployed `wearable-bridge` contract is three steps: `pair.create` (unauthenticated — the wearable has no identity yet — mints a challenge code plus a `pairingHandle`/`pairingSecret`), `pair.approve` (phone-authenticated, confirms consent only, returns no session), then `pair.poll` (unauthenticated, trust = `pairingHandle`+`pairingSecret`, this is what actually issues the wearable session token). `wearableBackend.js` had no wrapper for `pair.create`/`pair.poll` at all — by its own header comment, it only wrapped "phone JWT" operations, and `pair.create`/`pair.poll` are deliberately unauthenticated. Added both wrappers and rewrote `onPairRequest`/`approvePairing`/`unpair` to the corrected three-step flow, minting a fresh UUID per pairing attempt (the local companion-protocol device id, e.g. `hud_...`, is not UUID-shaped and fails `wearable-bridge`'s frame validation).

This changes the classification of the phone-side reference implementation from "COMPLETE + UNVERIFIED" to **"was actually BROKEN, now fixed and smoke-tested error-free."** Verification performed: fresh-tab browser load with zero console errors; graceful "not configured" fallback confirmed when no Supabase credentials are present (this dev environment's `.env` has none). Full click-through of pairing → scan → save → open-on-phone against a real signed-in user remains **blocked on live test credentials** — the same "Live QA: Pending credentials" gate Phase A itself never closed. This was not claimed as done.

A pre-existing, still-unresolved dead-code note: `phoneCompanion.js` defines `makeNonce()` but never calls it. Left as-is (not wired to invented behavior) pending a decision on whether it's meant for message-replay protection.

Full regression suite re-run after these fixes: lint (0 errors), static (0 fail), security (40/40), contract (128/128), companion (86/86 across all four sub-suites), wearable (14/14), privacy (16/16), bridge (19/19), all three builds green, artifact separation 25/25.
